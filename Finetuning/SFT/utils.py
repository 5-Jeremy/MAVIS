...
class Instructions:
    ...
    def get_response(response):
        ...
        print("response split: ", response.split(Instructions.response_split)[-1].strip())
...
def build_dataset(path, tokenizer, split='train', size=None):
    ds = load_dataset(path, split=split, data_dir="helpful-base")
    if size is not None:
    ...
    def tokenize_new(sample):
        sample['text'] = sample['chosen'] 
        split_text = sample['text'].split('\n\nAssistant:')
        sample['prompt'] = split_text[0] + '\n\nAssistant:'
        sample['response'] = split_text[1].split('\n\nHuman:')[0]
        sample["input_ids"] = tokenizer.encode(sample['prompt']+sample['response']) + [tokenizer.eos_token_id]
        sample["query"] = tokenizer.decode(sample["input_ids"])
        return sample
    
    ds_chosen = ds.map(tokenize, batched=False, num_proc=30)
    ds_concat = ds_chosen
    ds_concat = ds_concat.filter(lambda x: len(x["input_ids"]) <= 200 and len(x["input_ids"]) >= 8)
...
def build_dataset_summary(path, tokenizer, split='train', size=3000):
