...
class ModelArithmetic(PreTrainedModel):
    ...
    def next_token_speculative(self, continuation_tokens, 
        ...
        if len(to_sample_indices) > 0:
            ...
            # We need to first compute the base probability distribution
            # and the modified probability distribution, and then take the log of the ratio of the token probs. 
            # self.model_prediction_history[0][token_cnt].values() contains the logprobs from both models for the
            # most recently sampled token. The first value corresponds to the base distribution, which is what we
            # need. The modified distribution comes from self.logprobs_history
            if len(self.logprobs_history[0].keys()) > 1:
                token_cnt = len(self.logprobs_history[0].keys()) - 1
                sampled_token = new_tokens[0].item()
                all_base_logprobs = list(self.model_prediction_history[0][token_cnt].values())[0]
                all_modified_logprobs = self.logprobs_history[0][token_cnt]
                all_base_probs = torch.softmax(all_base_logprobs, dim=-1)
                all_modified_probs = torch.softmax(all_modified_logprobs, dim=-1)
                base_prob = all_base_probs[sampled_token]
                modified_prob = all_modified_probs[sampled_token]
                logprob_ratio = torch.log(modified_prob/base_prob)
                if self.logprob_ratio_sum is None:
                    self.logprob_ratio_sum = 0.0
                self.logprob_ratio_sum += logprob_ratio
    ...
    def generate_text(self, sentences, max_new_tokens=1024, stop_texts=None, batch_size=None,
        ...
        num_toks_generated = 0
        self.logprob_ratio_sum = 0
        while len(current_indices) > 0:
            ...
            num_toks_generated += len(next_tokens)

        generation_end_time = time.time()
        # print(f"Average tokens per second: {num_toks_generated / (generation_end_time - generation_start_time):.2f}")
        # Write the KL divergence to a CSV file so we can compute the average later
        with open(f"results/kl_divergence_{os.getenv('KL_file_suffix')}.csv", "a") as f:
            f.write(f"{self.logprob_ratio_sum}\n")
